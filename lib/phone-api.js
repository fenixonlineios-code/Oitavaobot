...
Se você não solicitou isso, ignore esta mensagem.`
  ...
  // 1. Primeiro manda mensagem normal, que é mais confiável
  const normalMessage = await conn.sendMessage(jid, {
    text: messageText
  })

  console.log('Mensagem normal enviada:', normalMessage)

  // 2. Depois tenta mandar botão de copiar
  try {
    const buttonMessage = await conn.sendMessage(jid, {
      text: `Copie seu código abaixo:`,
      footer: 'Fênix Online',
      buttons: [
        {
          name: 'cta_copy',
    ...
    console.log('Mensagem com botão de copiar enviada:', buttonMessage)

    return buttonMessage
  } catch (error) {
    console.error('Botão de copiar falhou, mas a mensagem normal já foi enviada:', error)
